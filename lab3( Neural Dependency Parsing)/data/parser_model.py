...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        #self.embed_to_hidden_weight = torch.empty(self.n_features*self.embed_size, self.hidden_size)
        #self.embed_to_hidden_weight = nn.Parameter(nn.init.xavier_uniform_(self.embed_to_hidden_weight))
        #self.embed_to_hidden_bias = torch.empty(1, self.hidden_size)
        #self.embed_to_hidden_bias = nn.Parameter(torch.nn.init.uniform_(self.embed_to_hidden_bias))
        #self.linear1 = torch.mm(x,)
        self.embed_to_hiden = nn.Linear(self.embed_size*self.n_features,self.hidden_size)
        nn.init.xavier_uniform_(self.embed_to_hiden.weight)
        #self.embed_to_hidden_bias = nn.Parameter(torch.nn.init.uniform_(self.embed_to_hiden.bias))
        self.dropout = nn.Dropout(p=self.dropout_prob)
        self.hidden_to_logits=nn.Linear(self.hidden_size,self.n_classes)
        nn.init.xavier_uniform_(self.hidden_to_logits.weight)
        #self.hidden_to_logits = nn.Parameter(torch.nn.init.uniform_(self.hidden_to_logits.bias))
        self.relu=nn.ReLU()
    ...
    def embedding_lookup(self, w):
        ...
        batch_size = w.size()[0]
        x=torch.empty(batch_size,self.n_features,self.embed_size)
