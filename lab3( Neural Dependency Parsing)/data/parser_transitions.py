...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        if len(unfinished_parses)>batch_size:
            mini = unfinished_parses[0:batch_size]
        ...
        #batch_dependencies = [None]*len(mini)
        while mini:
           cutindex = []
           transitions = model.predict(mini)
           #注意这里的变量cutindex需要每次都参与循环，因为每一次循环后会得到不同的需要删除的序号，每次循环之前应该置空
           #transitions函数也是参与循环的，需要注意
           for s in range(len(mini)):
               mini[s].parse_step(transitions[s])
               if not mini[s].buffer and len(mini[s].stack)==1:
                   cutindex.append(s)  #注意这里不能直接删掉index为s的元素，因为这样会导致在接下来的循环过程中序号整个前移然后匹配错误
                   dependencies[j*batch_size+index_dic[s]]=mini[s].dependencies
           # print(cutindex)
           for i in sorted(cutindex,reverse=True): #降序排列，防止删除前面的元素导致整个序号前移发生匹配错误
               del mini[i]
               del index_dic[i]
